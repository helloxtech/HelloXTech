...
class Utils {
	...
	public static function get_site_domain() {
		return str_ireplace( 'www.', '', parse_url( home_url(), PHP_URL_HOST ) );//phpcs:ignore WordPress.WP.AlternativeFunctions.parse_url_parse_url
	}

	public static function get_current_post_id() {
		if ( isset( \Elementor\Plugin::instance()->documents ) && null !== \Elementor\Plugin::instance()->documents->get_current() ) {
	...
	public static function get_ensure_upload_dir( $path ) {
		...
		foreach ( $files as $file ) {
			if ( ! file_exists( trailingslashit( $path ) . $file['file'] ) ) {
				...
				@ file_put_contents( trailingslashit( $path ) . $file['file'], $content );//phpcs:ignore Generic.PHP.NoSilencedErrors.Forbidden, WordPressVIPMinimum.Functions.RestrictedFunctions.file_ops_file_put_contents
			}
		}
	...
	public static function _unstable_file_get_contents( $file, ...$args ) {
		...
		return file_get_contents( $file, ...$args );//phpcs:ignore WordPressVIPMinimum.Performance.FetchingRemoteData.FileGetContentsUnknown
	}
