...
class FormSanitizeButtonId extends $e.modules.hookData.Dependency {
  ID_SANITIZE_FILTER = /[^\w]/g;
  getCommand() {
    return 'document/elements/settings';
  }
  ...
  apply(args) {
    ...
    // Re-render with old settings.
    if (formId.match(this.ID_SANITIZE_FILTER)) {
      const formIdView = container.panel.getControlView('button_css_id');
      formIdView.render();
      formIdView.$el.find('input').trigger('focus');
...
* This is supposed to sanitize Button CSS ID
* Must be removed when Elementor will do it */
var _formSanitizeButtonId = __webpack_require__(/*! ./form-sanitize-button-id */ "../modules/forms/assets/js/editor/hooks/data/form-sanitize-button-id.js");
...
class FormsModule extends elementorModules.editor.utils.Module {
  onElementorInit() {
    ...
    this.mailerlite = new MailerLite('twbb_form');
...
module.exports = elementorModules.editor.utils.Module.extend({
  ...
  getDataSettings(item) {
    const config = elementorTenweb.config.forms[item.field_type],
...
class Link extends $e.modules.document.CommandHistory {
  ...
  apply(args) {
    ...
    containers.forEach(( /** Container */container) => {
      ...
      const elementModel = elementorTenweb.modules.globalWidget.addGlobalWidget(data.template_id, data),
  ...
  async apply(args) {
    ...
    containers.forEach(( /** Container */container) => {
      const id = container.model.get('templateID'),
        elementModel = elementorTenweb.modules.globalWidget.createGlobalModel(id, data[id]);
      $e.run('document/elements/create', {
        container: container.parent,
        model: {
          id: elementor.helpers.getUniqueID(),
...
class GlobalWidgetLoadTemplates extends $e.modules.hookData.After {
  ...
  apply() {
    ...
    Object.entries(elementorTenweb.config.widget_templates).forEach(_ref => {
      let [id, data] = _ref;
      elementorTenweb.modules.globalWidget.addGlobalWidget(id, data);
...
class Module extends elementorModules.editor.utils.Module {
  ...
  getGlobalModels(id) {
    elementorCommon.helpers.softDeprecated('elementorTenweb.modules.globalWidget.getGlobalModels( id )', '3.5.0', '$e.data.getCache( `document/global/global-widget/templates/${ id }` )');
    return $e.data.getCache(this.component, `document/global/global-widget/templates/${id}`);
  }
  saveTemplates() {
    elementorCommon.helpers.softDeprecated('elementorTenweb.modules.globalWidget.saveTemplates()', '3.5.0', "$e.internal( 'document/global/save-templates' )");
    $e.internal('document/global/save-templates');
  }
  requestGlobalModelSettings(globalModel, callback) {
    elementorCommon.helpers.softDeprecated('elementorTenweb.modules.globalWidget.requestGlobalModelSettings()', '3.5.0', "$e.data.get( 'document/global/templates' )");
    $e.data.get('document/global/templates', {
      ids: globalModel.id
    }).then(data => {
      callback(data);
...
class Model extends ElementModel {
  ...
  initSettingsFromTemplate() {
    const id = this.get('templateID'),
      ...
      elementModel = elementorTenweb.modules.globalWidget.createGlobalModel(id, data);
...
module.exports = ElementEditorModule.extend({
  ...
  onSectionActive() {
    return elementorTenweb.ajax.addRequest('get_stripe_tax_rates', {
      success: data => {
...
class PopupSave extends $e.modules.hookData.After {
  ...
  apply() {
    const settings = {};
    jQuery.each(elementorTenweb.modules.popup.displaySettingsTypes, (type, data) => {
...
class PopupRemoveTriggers extends $e.modules.hookUI.After {
  ...
  removePanelFooterSubmenuItems() {
    const displaySettingsTypes = elementorTenweb.modules.popup.displaySettingsTypes;
  ...
  addPublishTabs() {
    ...
    jQuery.each(module.displaySettingsTypes, (type, data) => {
      ...
      component.addTab(type, {
        ...
        image: elementorTenweb.config.urls.modules + `popup/assets/images/${type}-tab.svg`
      });
    });
  }
...
class ThemeBuilderSaveConditions extends $e.modules.hookData.After {
  ...
  apply() {
    const {
      conditionsModel
    } = elementorTenweb.modules.themeBuilder;
...
class ThemeBuilderModule extends elementorModules.editor.utils.Module {
  __construct() {
    ...
    Object.defineProperty(elementorTenweb.config, 'theme_builder', {
      get() {
...
class WoocommerceCreateWidgetActivateSettingsModal extends $e.modules.hookData.After {
  ...
  getId() {
    return 'elementor-tenweb-woocommerce-create-widget-activate-settings-modal';
  }
  getContainerType() {
    return 'column';
  }
  getConditions(args, container) {
    return Object.prototype.hasOwnProperty.call(elementorTenweb.modules.woocommerce.pageSettingsWidgets, container.model.get('widgetType'));
  }
  apply(args, container) {
    elementorTenweb.modules.woocommerce.onCreateWidget(container);
  }
}
...
class WoocommerceDeleteWidgetDeactivateSettingsModal extends $e.modules.hookData.After {
  ...
  getId() {
    return 'elementor-tenweb-woocommerce-delete-widget-deactivate-settings-modal';
  }
  getContainerType() {
    return 'widget';
  }
  ...
  apply(args, container) {
    elementorTenweb.modules.woocommerce.onDeleteWidget(container);
  }
}
...
class WoocommerceSaveShowModal extends $e.modules.hookData.After {
  ...
  getId() {
    return 'elementor-tenweb-woocommerce-save-show-modal';
  }
  getConditions(args) {
    return args.status && -1 !== ['private', 'publish'].indexOf(args.status);
  }
  apply() {
    elementorTenweb.modules.woocommerce.onUpdateDocument();
  }
}
...
class WoocommerceModule extends elementorModules.editor.utils.Module {
  constructor() {
    ...
    this.pageSettingsWidgets = {
      'twbb_woocommerce-checkout-page': {
      ...
      'twbb_woocommerce-purchase-summary': {
  ...
  renderMockNotices(noticeElements) {
    ...
    elementorTenweb.ajax.addRequest('woocommerce_mock_notices', {
      data: {
  ...
  onUpdateDocument() {
    ...
    if (postId === elementorTenweb.config.woocommerce.woocommercePages[lastWidgetCreatedOptions.woocommercePageName]) {
      return;
    }
  ...
  onConfirmModal(lastWidgetCreatedOptions) {
    elementorTenweb.ajax.addRequest('woocommerce_update_page_option', {
      data: {
