...
class Elementor_Post_Query {
    ...
    //10Web Customization
    protected function get_post_taxonomies($post_type) {
        $taxonomy_filter_args = [
            'show_in_nav_menus' => true,
            'object_type' => [$post_type],
        ];
        $taxonomy_list = get_taxonomies($taxonomy_filter_args);
	...
	protected function set_terms_args() {
		...
		$this->build_terms_query_include( 'include_term_ids_tags' );
		$this->build_terms_query_exclude( 'exclude_term_ids' );
		$this->build_terms_query_exclude( 'exclude_term_ids_tags' );

        //10Web Customization
        $taxonomy_list = $this->get_post_taxonomies($post_type);
        foreach ($taxonomy_list as $taxonomy) {
            $this->build_terms_query_include( 'include_taxonomy_' . $post_type . '_' . $taxonomy);
        }
        foreach ($taxonomy_list as $taxonomy) {
            $this->build_terms_query_exclude( 'exclude_taxonomy_' . $post_type . '_' . $taxonomy);
        }
	}
