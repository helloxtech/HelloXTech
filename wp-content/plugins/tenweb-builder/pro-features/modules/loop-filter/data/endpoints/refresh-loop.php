...
use Tenweb_Builder\ElementorPro\Modules\LoopFilter\Data\Interfaces\Endpoint;
use Tenweb_Builder\ElementorPro\Modules\LoopFilter\Module;
...
class Refresh_Loop extends Base implements Endpoint {
	...
	private function permission_callback( $request ): bool {
		...
		$document = \Elementor\Plugin::instance()->documents->get( $data['post_id'] );
	...
	private function create_widget_instance_from_db( $post_id, $widget_id ) {
		...
		return \Elementor\Plugin::instance()->elements_manager->create_element_instance( $widget_data );
	}
	...
	public function get_updated_loop_widget_markup( \WP_REST_Request $request ): array {
		...
		$loop_filter = \Elementor\Plugin::instance()->modules_manager->get_modules( 'loop-filter' );

		$loop_filter->register_widget_filter( $data['widget_id'], $data['widget_filters'] );

		if ( $this->is_edit_mode( $data['post_id'] ) ) {
			$widget_instance = \Elementor\Plugin::instance()->elements_manager->create_element_instance( $data['widget_model'] );
		} else {
