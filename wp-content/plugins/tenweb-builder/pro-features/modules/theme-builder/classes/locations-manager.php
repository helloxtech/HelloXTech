<?php
namespace Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Classes;

use Tenweb_Builder\ElementorPro\Core\Utils;
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Documents\Theme_Document;
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Module;
...
class Locations_Manager {
	...
	public function inspector_log( $args ) {
		$inspector_enabled = method_exists( \Elementor\Plugin::instance()->inspector, 'is_enabled' ) && \Elementor\Plugin::instance()->inspector->is_enabled();
		...
		if ( isset( $args['template'] ) ) {
			$title[] = \Elementor\Plugin::instance()->inspector->parse_template_path( $args['template'] );
		}

		$title = implode( ' > ', $title );

        \Elementor\Plugin::instance()->inspector->add_log( 'Theme', $title, $url );
	}
