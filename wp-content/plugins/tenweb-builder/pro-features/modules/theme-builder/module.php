<?php
namespace Tenweb_Builder\ElementorPro\Modules\ThemeBuilder;
...
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\AdminMenuItems\Theme_Builder_Menu_Item;
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Classes;
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Documents\Single;
use Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Documents\Theme_Document;
...
class Module extends Module_Base {
	...
	public function localize_settings() {
        \Elementor\Plugin::instance()->modules_manager->get_modules( 'dev-tools' )->deprecation->deprecated_function( __METHOD__, '3.1.0' );
	...
	public function create_new_dialog_types( $types ) {
		...
		foreach ( $types as $type => $label ) {
			$document_type = \Elementor\Plugin::instance()->documents->get_document_type( $type );
			...
			<div class="elementor-form-field__select__wrapper">
				...
				>
					...
					foreach ( $post_types as $post_type => $label ) {
						$doc_type = \Elementor\Plugin::instance()->documents->get_document_type( $post_type );
	...
	public function on_elementor_editor_init() {
        \Elementor\Plugin::instance()->common->add_template( __DIR__ . '/views/panel-template.php' );
	}
	...
	private function register_admin_menu( MainMenu $menu ) {
		$menu->add_submenu( [
			...
			'menu_slug' => \Elementor\Plugin::instance()->app->get_settings( 'menu_url' ),
			'index' => 30,
		] );
	}
		...
		if ( self::ADMIN_LIBRARY_TAB_GROUP === $current_tab_group ) {
			...
			$admin_notices->print_admin_notice( [
				...
				'button' => [
					...
					'url' => \Elementor\Plugin::instance()->app->get_settings( 'menu_url' ),
				],
			] );
		}
	...
	public function __construct() {
		...
		if ( \Elementor\Plugin::instance()->experiments->is_feature_active( 'admin_menu_rearrangement' ) ) {
