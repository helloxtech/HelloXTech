<?php
namespace Tenweb_Builder\ElementorPro\Modules\ThemeBuilder\Documents;
...
abstract class Single_Base extends Archive_Single_Base {
	...
	public function get_depended_widget() {
		return \Elementor\Plugin::instance()->widgets_manager->get_widget_types( 'theme-post-content' );
	}

	public function get_elements_data( $status = DB::STATUS_PUBLISH ) {
		$data = parent::get_elements_data();

		if ( \Elementor\Plugin::instance()->preview->is_preview_mode() && self::get_property( 'location' ) === Module::instance()->get_locations_manager()->get_current_location() ) {
			$has_the_content = false;

			$depended_widget = $this->get_depended_widget();

            \Elementor\Plugin::instance()->db->iterate_data( $data, function( $element ) use ( &$has_the_content, $depended_widget ) {
